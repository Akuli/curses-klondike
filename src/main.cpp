...
static void new_game(Klon *kln, SelMv *selmv)
{
	kln->init();
	selmv->ismv = false;
	selmv->sel.place = CardPlace(CardPlace::STOCK);
	selmv->sel.card = NULL;
}
...
static bool handle_key(Klon *kln, SelMv *selmv, int k, Args ar, const char *argv0)
{
	...
	if (k == 's' && !selmv->ismv) {
		...
		selmv_byplace(*kln, selmv, CardPlace(CardPlace::DISCARD));
	...
	if (k == '\n') {
		...
		else if (selmv->sel.place.kind == CardPlace::STOCK)
			kln->stock2discard(ar.pick);
	...
	if ('1' <= k && k <= '7') {
		selmv_byplace(*kln, selmv, CardPlace(CardPlace::TABLEAU, k - '1'));
		return true;
	}
