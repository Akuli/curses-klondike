...
static void draw_the_klon(WINDOW *win, Klon kln, Sel sel, bool moving, bool color, DiscardHide dh, int dscxoff)
{
	...
	draw_card(win, kln.stock, ui_x(0, w), ui_y(0, h), sel.place.kind == CardPlace::STOCK, color);

	int nshowdis = kln.discardshow;
	if (sel.place.kind == CardPlace::DISCARD && moving)  // user is moving a detached card, and it's in the discard
		nshowdis++;
	...
	for (Card *crd = card_tops(kln.discard, nshowdis); crd; (crd = crd->next), (x += dscxoff)) {
		...
		draw_card(win, &crdval, x, ui_y(0, h), sel.place.kind == CardPlace::DISCARD && !crd->next, color);
	}

	if (!kln.discard)   // nothing was drawn, but if the discard is selected, at least draw that
		draw_card(win, NULL, ui_x(1, w), ui_y(0, h), sel.place.kind == CardPlace::DISCARD, color);

	// foundations are similar to discard
	for (int i=0; i < 4; i++)
		draw_card(win, card_top(kln.foundations[i]), ui_x(3+i, w), ui_y(0, h), sel.place == CardPlace(CardPlace::FOUNDATION, i), color);

	// now the tableau... here we go
	for (int x=0; x < 7; x++) {
		if (!kln.tableau[x]) {
			// draw a border if the tableau item is selected
			draw_card(win, NULL, ui_x(x, w), ui_y(1, h), sel.place == CardPlace(CardPlace::TABLEAU, x), color);
			continue;
		}
...
static DiscardHide decide_what_to_hide(SelMv selmv, bool cmdlnopt)
{
	...
	if (selmv.mv.src.kind == CardPlace::DISCARD && selmv.mv.dst.kind == CardPlace::DISCARD)
		return DiscardHide::SHOW_LAST_ONLY;
	if (selmv.mv.src.kind == CardPlace::DISCARD)
